...
mod tests {
    ...
    fn test_value_to_string_null() {
        let value = ValueRefX {
            value: ValueRefXValue::Null,
            value_type: 0,
        };
        assert_eq!(value_to_string(&value).unwrap(), "");
    }

    #[test]
    fn test_value_to_string_int() {
        let value = ValueRefX {
            value: ValueRefXValue::Int(42),
            value_type: 0,
        };
        assert_eq!(value_to_string(&value).unwrap(), "42");
    }

    #[test]
    fn test_value_to_string_double() {
        let value = ValueRefX {
            value: ValueRefXValue::Double(3.14),
            value_type: 0,
        };
        assert_eq!(value_to_string(&value).unwrap(), "3.14");
    }

    #[test]
    fn test_value_to_string_text() {
        let value = ValueRefX {
            value: ValueRefXValue::Text(b"hello".to_vec()),
            value_type: 0,
        };
        assert_eq!(value_to_string(&value).unwrap(), "hello");
    }

    #[test]
    fn test_value_to_string_blob() {
        let value = ValueRefX {
            value: ValueRefXValue::Blob(vec![0, 1, 2]),
            value_type: 0,
        };
        assert_eq!(value_to_string(&value).unwrap(), "");
    }
    ...
    fn test_value_to_json_null() {
        ...
        assert_eq!(value_to_json(&value).unwrap(), serde_json::Value::Null);
    }
    ...
    fn test_value_to_json_double() {
        ...
        match value_to_json(&value).unwrap() {
    ...
    #[test]
    fn test_value_to_json_invalid_float() {
        let value = ValueRefX {
            value: ValueRefXValue::Double(f64::NAN),
            value_type: 0,
        };
        assert!(matches!(
            value_to_json(&value),
            Err(ExportError::InvalidFloat(_))
        ));
    }
}
