...
def test_docgen_without_extension(solite_cli, tmp_path):
    """docgen must work without --extension (regression: it used to fail
    with "no such table: solite_docs.solite_docs_loaded_functions")."""
    doc = tmp_path / "doc.md"
    doc.write_text("# Demo\n\n```sql\nSELECT 1 + 1;\n```\n")

    result = solite_cli(["docgen", str(doc)], cwd=tmp_path)
    assert result.success, result.stderr
    assert "no such table" not in result.stderr
    assert "-- 2" in result.stdout


def test_docgen_output_file(solite_cli, tmp_path):
    doc = tmp_path / "doc.md"
    doc.write_text("# Demo\n\n```sql\nSELECT 'hello' AS greeting;\n```\n")

    result = solite_cli(
        ["docgen", str(doc), "--output", "out.md"], cwd=tmp_path
    )
    assert result.success, result.stderr
    assert "'hello'" in (tmp_path / "out.md").read_text()


def test_docgen_skips_non_sql_blocks(solite_cli, tmp_path):
...
def test_docgen_sameline_statements(solite_cli, tmp_path):
    ...
    result = solite_cli(
        ["docgen", str(doc), "--output", "out1.md"], cwd=tmp_path
    )
    assert result.success, result.stderr
    ...
    result = solite_cli(
        ["docgen", str(tmp_path / "out1.md"), "--output", "out2.md"],
        cwd=tmp_path,
    )
    assert result.success, result.stderr
...
def test_docgen_adjacent_statements_no_blank_line(solite_cli, tmp_path):
...
def test_docgen_comment_breakout(solite_cli, tmp_path):
...
def test_docgen_multiline_value_stays_commented(solite_cli, tmp_path):
...
def test_docgen_error_report(solite_cli, tmp_path):
...
def test_docgen_anchor_idempotent(solite_cli, tmp_path):
    """Re-running docgen over its own output keeps exactly one
...
def test_docgen_anchor_self_heals(solite_cli, tmp_path):
...
def test_docgen_unicode_table_alignment(solite_cli, tmp_path):
...
def test_docgen_no_json_prefix(solite_cli, tmp_path):
...
def test_docgen_gfm_table_and_strikethrough(solite_cli, tmp_path):
...
def test_docgen_frontmatter(solite_cli, tmp_path):
...
def test_docgen_no_reformatting_churn(solite_cli, tmp_path):
...
def test_docgen_nested_code_block(solite_cli, tmp_path):
...
def test_docgen_empty_sql_block(solite_cli, tmp_path):
...
def test_docgen_empty_sql_block_crlf(solite_cli, tmp_path):
...
def test_docgen_blockquoted_sql_block_untouched(solite_cli, tmp_path):
...
def test_docgen_multirow_table(solite_cli, tmp_path):
    doc = tmp_path / "doc.md"
...
def test_docgen_zero_rows(solite_cli, tmp_path):
    doc = tmp_path / "doc.md"
    doc.write_text("# Demo\n\n```sql\nSELECT 1 WHERE 0;\n```\n")
...
def test_docgen_error_writes_no_output_file(solite_cli, tmp_path):
    doc = tmp_path / "doc.md"
    doc.write_text("# Demo\n\n```sql\nSELECT * FROM no_such_table;\n```\n")
...
def test_docgen_in_place_regeneration_stable(solite_cli, tmp_path):
    ...
    result = solite_cli(
        ["docgen", str(doc), "--output", str(doc)], cwd=tmp_path
    )
    assert result.success, result.stderr
    first = doc.read_text()
...
def test_docgen_extension_documented(solite_cli, tmp_path, docs_extension):
    ...
    result = solite_cli(
        ["docgen", str(doc), "--extension", str(docs_extension)],
        cwd=tmp_path,
    )
    assert result.success, result.stderr
...
def test_docgen_extension_undocumented(solite_cli, tmp_path, docs_extension):
...
def test_docs_inline_spelling_removed(solite_cli, tmp_path):
    """`solite docs inline` was renamed to `solite docgen` — clean break,
    no alias."""
    doc = tmp_path / "doc.md"
    doc.write_text("# Demo\n\n```sql\nSELECT 1;\n```\n")
...
def test_docgen_expect_error_directive(solite_cli, tmp_path):
    """A `-- @expect-error` statement inlines the actual error message; the
    directive is stripped from the output and in-place regeneration is
    byte-stable (the generated `-- error:` line doubles as the marker)."""
    doc = tmp_path / "doc.md"
    doc.write_text("# Demo\n\n```sql\n-- @expect-error\nselect * from missing;\n```\n")
    ...
    assert result.success, result.stderr
    first = doc.read_text()
    assert "-- error: no such table: missing" in first
    assert "@expect-error" not in first
